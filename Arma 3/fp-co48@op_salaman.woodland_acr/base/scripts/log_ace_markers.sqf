...
if (_closeNum == 1) then {
    if (time < 1) then { // only at mission briefing
        [[
            ...
            {
                systemChat format ["SERVER: %1 placed a '%2' at %3",
                name (_this select 0),
                if ((_this select 1) != "") then {getText (configFile >> "cfgMarkers" >> (_this select 1) >> "name") + " marker"} else {"marker"},
                if (side player == side (_this select 0)) then {_this select 2} else {"<redacted>"}]
            }
