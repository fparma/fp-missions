...
if (count _this in [1,2]) exitWith {
	...
	_id = _laptop addAction ["<t color='#ff8a00'>\\ ARM DEVICE //</t>", "fp_scripts\laptop.sqf",0, 9, false, true, "", "side player == opfor && 
		isNil 'FP_end' && ((_target getVariable ['fp_inUse',0]) != 1 && player distance distanceLogic < 4.5)"];
...
if (!T8_abort) then 
{
	...
	while { (!_finished && alive player && animationState player == _state) && (!(player getVariable ["AGM_unconscious", false])) } do
	{
		if (time > _delay) then {
...
FP_end = "TRUCKARMED"; publicVariable "FP_end";
