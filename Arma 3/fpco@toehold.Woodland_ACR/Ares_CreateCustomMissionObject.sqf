/*
	...
	Parameters:
		...
		1 - Code Block - The block of code to run that will create the custom object. This code block should return an object or an array of objects to add to the Curator after completion (or objNull). 
		The position to create the object at can be accessed using '_this select 0' in the code block.
...
Ares_CustomMissionObjectData pushBack [
	"allowDamage false, 50m",
	{
		{_x allowDamage false} forEach (nearestObjects [_this select 0, ["All"], 50]);
	}
];