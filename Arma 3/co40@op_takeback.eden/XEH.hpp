...
class Extended_InitPost_EventHandlers {
  class CAManBase {
    init = "if (local (_this select 0)) then {[(_this select 0), 'shut_up'] call ace_common_fnc_muteUnit}; [_this select 0] call FP_useTracers";
  };
  class Air {
    init="(_this select 0) call FP_attachRopeEH"
  };
};
