...
switch toLower(worldName) do {		
        case "chernarus": {
                ...
                [] spawn compile preprocessFileLineNumbers "scripts\ALICE2_houseEffects.sqf";
        ...
        case "zargabad": {
		        ...
		        case "emita": {
                BIS_alice_mainscope setvariable ["trafficDistance",1000];
				BIS_alice_mainscope setVariable ["civilianCount","round (0.6 * (sqrt %1))"]; 
                BIS_alice_mainscope setvariable ["spawnDistance",600];
                BIS_alice_mainscope setVariable ["townsFaction",["CIV","CIV_RU"]];
                [] spawn compile preprocessFileLineNumbers "scripts\ALICE2_houseEffects.sqf";
        };
...
CUEL_CIV_KILLED={
	...
	if (_killer == _target) then {
		...
		{
			if ((isPlayer (driver (vehicle _x))) && (speed _x > 3)) exitWith {_killer = _x};
		}foreach _vehArr;
	};

	if ( (side _killer) == WEST) then{
		[nil, nil, rHINT, format ["%1 has killed a civilian! this is unacceptable.",(name _killer)]]call RE;
	};
};

[BIS_alice_mainscope,"ALICE_civilianinit",[
	{_this addeventhandler ["killed",{_this spawn CUEL_CIV_KILLED}]},
        {_this setSkill 0},
        {
