...
while {true} do {
	...
	if (damage _unit>0.28) then {
		...
		_claw="";
		if (damage _unit>0.28) then {_claw="zombie_scripts\claw1.paa"};
		if (damage _unit>0.59) then {_claw="zombie_scripts\claw2.paa"};
		if (damage _unit>0.9) then {_claw="zombie_scripts\claw3.paa"};
