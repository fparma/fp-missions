...
MON_GetIn_NearestVehicles = {
	...
	{
		if ( (_x!= vehicle _x && "StaticWeapon" countType [vehicle (_x)] == 0) || !alive _x || !canmove _x) then {_units = _units-[_x];};
	}foreach _units;
	
	_unitsIn = _units;
	
	if ( (count _units) > 0) then {	
		_air = [_npc,250] call MON_NearestsAirTransports;
		_units = [_grpid, _units, _air, false] call MON_selectvehicles;				
	};
	
	if ( (count _units) > 0) then {	
		_vehicles = [_npc,250] call MON_NearestsLandTransports;
...
MON_GetIn_NearestStatic = {
	...
	{
		if ( _x == vehicle _x && alive _x && canmove _x) then {
	...
	
  	{
		...
		if ( _vehgrpid == _grpid || (_emptypositions > 0 && _any)) then {
			...
			if ( (count _unitsIn) > 0) then {			
				...
				if (KRON_UPS_Debug>0 ) then {player sidechat format["%1: Moving %3 units into %2 with %4 positions",_grpid,typeof _vehicle,count _unitsIn,_emptypositions]}; 				
			};
		};		
		_index 	= _index  + 1;
		sleep 0.05;
...
MON_UnitsGetIn = {
	...
	{		
		...
		{
			//if (KRON_UPS_Debug>0 ) then {player sidechat format["%1: saliendo statica %2",_grpid, typeof _veh]}; 	
...
MON_avoidDissembark = {
	...
	//waituntil {_npc != vehicle _npc || !alive _npc || !canmove _npc};	
	_npc stop false;
	sleep 0.5;	
...
MON_NearestLandTransport = {
		...
		
		{
			...
			if (!locked _x  && canMove _x && _transportSoldier >= count (units _npc) && (_drivercount > 0 || side _npc == side _x )) exitwith {_vehicle = _x;};
		}foreach _OCercanos;
...
MON_NearestsLandCombat = {
	...
	_distance = _this select 1;					
			...
			
		{
			...
			if (!locked _x && _Gunnercount > 0 && canMove _x && (_drivercount > 0 || side _npc == side _x )) then { _vehicles = _vehicles + [[_x,_emptypositions]];};
		}foreach _OCercanos;
...
MON_NearestsStaticWeapons = {
	...
	_distance = _this select 1;					
			...
			
		{
			...
			if (!locked _x && alive _x && _emptypositions > 0) then { _vehicles = _vehicles + [[_x,_emptypositions]];};
		}foreach _OCercanos;
...
MON_GetOutDist = {
	...
	_dist = [_vehpos,_targetpos] call KRON_distancePosSqr;
	//if (KRON_UPS_Debug>0) then {player sidechat format["%1: Getoutdist dist=%1 atdist=%2 ",_dist, _atdist]}; 
...
MON_doGetOut = {	
	...
	waituntil {!alive _vehicle || ((velocity _vehicle select 0) <= 0 && (velocity _vehicle select 1) <= 0 )
							 || ( "Air" countType [_vehicle]>0 && ((position _vehicle) select 2) <= 1)};	
	sleep 1;
	//if (KRON_UPS_Debug>0 ) then { player globalchat format["%1 GETOUT FROM %2",_npc, TYPEOF _vehicle];};	
...
//	<- 	_atdist:  distance for doing paradrop or landing
MON_doParadrop = {
	private["_heli","_npc","_getout" ,"_gunner","_targetpos","_helipos","_dist","_index","_grp","_wp","_targetPosWp","_targetP","_units"];				
	...
	//if (KRON_UPS_Debug>0 ) then {player globalchat format["_targetPos=%1 _atdist=%2 _flyingheigh=%3",_targetPos,_atdist,_flyingheigh];};
	...
	while { (_dist >= _atdist || _dist < 10) && alive _heli && canmove _heli && count crew _heli > 0} do {			
		...
		_dist = [_helipos,_targetPosWp] call KRON_distancePosSqr;
	...
	//Jump
	if (((position _heli) select 2) >= 55 && !surfaceIsWater _helipos ) then { 			
			
		if (KRON_UPS_Debug>0 ) then {hintsilent format["doing paradrop high %1 dist=%2",(position _heli) select 2,_dist,_atdist];};
		...
		if (alive _npc && canmove _npc) then {
			...
			{if (alive _x && canmove _x) exitwith { _x move _targetPosWp;}}foreach _units;
		};
	} else {
		//land
		if ( !surfaceIsWater _helipos && ((random 100)<20 || !canmove _heli)) then {			
			[_heli] call MON_landHely;				
		} 
		else {				
...
//Lands hely	
MON_landHely = {
	private["_heli","_npc","_crew","_NearestEnemy","_timeout"];				
	_heli = _this select 0;
	_crew =[];
	_timeout = 0;
	...
	//Waits until velocity and heigh are good for getting out
	_timeout = 60 + time;
	waituntil {!alive _heli || time > _timeout || ((velocity _heli select 2) <= 1 && ((position _heli) select 2) <= 2)};
	
	if (((position _heli) select 2) > 5) exitwith { 
	...
	//if (KRON_UPS_Debug>0 ) then { player globalchat format["%1 landing before doing getout %2",typeof _heli, landResult _heli];};
	...
	{
		waituntil {vehicle _x == _x || !canmove _x || !alive _x || movetofailed _x  }; 	
	}forEach _crew;	
...
MON_HeliStuckcontrol = {
	...
	while {	alive _heli } do {
		//_lastpos = position _heli;		
		sleep 10;		
		if ((velocity _heli select 0) <= 1 && (velocity _heli select 1) <= 1 && (velocity _heli select 2) <= 1 && ((position _heli) select 2) >= 20) then {
...
MON_moveNearestBuildings = {
	...
	{
		if (unitReady _x && _x == vehicle _x && canmove _x && alive _x) then {_bldunitsin = _bldunitsin + [_x]}
	}foreach _units;
...
MON_movetoBuilding = {
	...
	if (moveToCompleted _npc &&  _timeout > time && alive _npc && canmove _npc) then {			
		...
		if (_dist < 3) then {
			//_npc setpos (_bld buildingPos (_altura));
			sleep 0.1;
