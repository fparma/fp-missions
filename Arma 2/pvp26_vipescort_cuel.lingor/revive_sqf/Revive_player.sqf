...
//_ACE_mod					= _revive_array select 102;		
...
_medVclArray = ["M113Ambul_TK_EP1","M113Ambul_UN_EP1",
				"HMMWV_Ambulance","HMMWV_Ambulance_CZ_DES_EP1","HMMWV_Ambulance_DES_EP1",
				"BASE_WarfareBFieldhHospital",
				"MASH","MASH_EP1",
				"BMP2_Ambul_Base",
				"GAZ_Vodnik_MedEvac",
				"Mi17_medevac_base",
				"TK_WarfareBFieldhHospital_Base_EP1","TK_WarfareBFieldhHospital_EP1",
				"US_WarfareBFieldhHospital_Base_EP1","US_WarfareBFieldhHospital_EP1",
				"UH60M_MEV_EP1"];
...
// Unit alive
	...
	{	
		...
		{	
			...
			//set _name unconscious if riding in a destroyed vcl - 180710
		...
		{	
			...
			if (_c == 0 && !r_carry_sqf && (_nearest_unit getVariable "NORRN_AIunconscious")) then {
				...
				{
					Norrn_reviveAction = _name addAction ["Revive", "revive_sqf\heal.sqf",[_name, _nearest_unit], 0, false, true];
					_c = 1;		
				};
			};
		...
		if (_goto_revive == 0 && !isplayer _name && (count (nearestObjects [_name, [_can_be_revived, _can_be_revived_2], 4])) > 1 ) then {
				...
				_name isKindOf _medic_2      && _medpacks  == 1 && (_name getVariable "NORRN_medpacks") > 0) then
			{
				...
				{ 	
					[0,0,0,[_name, _nearest_unit]] execVM "revive_sqf\heal.sqf";
				};
			};
		};
...
// Unit falls unconscious	
	...
	{	
		...
		if (!isNull _bag) then 
		{
			_bagMags  = getMagazineCargo _bag;
			_bagWeps  =  getWeaponCargo _bag;
		...
		//publicEH
		norrnRNoDam = _name;
		publicVariable "norrnRNoDam";
		_name switchMove "AmovPpneMstpSnonWnonDnon_healed"; 
		_name allowDamage false; 
		...
		{	
			...
			if (!isNull (unitBackpack _name)) then 
			{	
				removeBackpack _name;
			};
			if (_bagType != "") then
			{	
				_name addBackpack _bagType;
				sleep 0.1;
				_bag = unitBackpack _name;
		...
		// Added for ACE2 10/01/10 rucks
		if (!NORRN_R_SP && _hasruck) then 
		{
			if (!isNil "_ruckmags") then {
				_name setvariable ["ACE_RuckMagContents", _ruckmags];
			};
			if (!isNil "_ruckweps") then {
				_name setvariable ["ACE_RuckWepContents", _ruckweapons];
			};
		};
		...
		if (_name == player) then {unconscious_body = _name};
		_name playActionNow "agonyStart";
		
		sleep 4;
		norrnRAlie = _name;
		publicVariable "norrnRAlie";
		_name switchMove "ainjppnemstpsnonwrfldnon";
	...
	{	
		...
		{	
			...
			{			
				...
				if (_d == 22 && count (nearestObjects [_name, [_can_be_revived, _can_be_revived_2], 4.5]) == 1) then 
				{	
					// public var EH
					norrnRrTF = _name;
					publicVariable "norrnRrTF";
					_name playMove "ainjppnemstpsnonwrfldnon_rolltofront"
				};
			};
			sleep 0.3;
			...
			{	
				...
				{	
					...
					//hintSilent format ["uncMates: %1", _mySaviour getVariable "NORRN_heyImBusy"];
					_tryAgain = false;
				...
				{	
					...
					//publicEH
					norrnDam = _name;
					publicVariable "norrnDam";
					_name switchMove "AmovPpneMstpSnonWnonDnon_healed"; 
					_name allowDamage true; 
...
// Respawn at base
	...
	{	
		...
		if (count(_name getVariable "NORRN_uncPos") != 0) then {[_name,_magazines,_weapons,_dir,_bag,_bagMags,_bagWeps] spawn Norrn_createBody};
