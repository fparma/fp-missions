...
if(not isNil "bon_ied_greenzone") then {
	...
	// added by cuel.
	_roads = _roads -  ((getmarkerpos "respawn_east") nearRoads (500));
	_roads = _roads -  ((getmarkerpos "no_ieds") nearRoads (800));
	_roads = _roads -  ((getmarkerpos "no_ieds_1") nearRoads (1000));
	_roads = _roads -  ((getmarkerpos "no_ieds_2") nearRoads (2000));
	_roads = _roads -  ((getpos assassinLoc) nearRoads (800));
	// --------------
};
