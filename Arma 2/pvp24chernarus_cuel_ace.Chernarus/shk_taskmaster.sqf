...
/* == COMMON =================================================================================== */
  ...
  SHK_Taskmaster_addTask = {
    ...
    */
    ...
    if taskMasterDEBUG then { diag_log format ["SHK_Taskmaster> addTask: %1, %2, %3, %4",_name,_marker,_state,_dest]};
    {
  ...
  SHK_Taskmaster_checkCond = {
    ...
    */
    ...
    if (!isNil "_cond") then {
      if taskMasterDEBUG then { diag_log format ["SHK_Taskmaster> typename condition: %1",typename _cond]};
    ...
    private "_name";
    {
      ...
      if (_name call SHK_Taskmaster_hasTaskLocal) then {
        if ([_name,(_x select 5)] call SHK_Taskmaster_hasStateChanged) then {
          if taskMasterDEBUG then { diag_log format ["SHK_Taskmaster> handleEvent calling updateTask: %1",_name]};
          _x call SHK_Taskmaster_updateTask;
        };
      } else {
        if taskMasterDEBUG then { diag_log format ["SHK_Taskmaster> handleEvent calling addTask: %1",_name]};
          ...
          _handle = _x;
          {
            if (_handle in (simpletasks _x)) then {
              ...
              if (_x == player) then {
                ...
                if (count _marker > 0) then {
                  if (_state in ["succeeded","failed","canceled"]) then {
                    if taskMasterDEBUG then { diag_log format ["SHK_Taskmaster> updateTask deleting marker: %1, state: %2",_marker,_state]};
                    if (typename (_marker select 0) == typename "") then {
                      _marker = [_marker];
                    };
                    {
...
if !isdedicated then {
  ...
  */
  [] spawn {
    ...
    if taskMasterDEBUG then {diag_log format ["SHK_Taskmaster> Tasks received first time: %1",SHK_Taskmaster_Tasks]};
    private "_sh";
