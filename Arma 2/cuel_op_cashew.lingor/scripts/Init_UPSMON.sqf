...
KRON_UPS_searchVehicledist = 200; // 700, 900  
							...
							if (!(_target in _targets ) || isnull _target || !alive _target) then {
								_target = objnull;
								{
									...
									if (!isnil "_targetPos") then {
										...
										if (alive _auxtarget && !(_auxtarget iskindof "Air") && (round([position 
_arti,_targetPos] call KRON_distancePosSqr) <= _range) && (_arti distance _targetPos) > 800) then {
											_target = _auxtarget;
											//Must check if no friendly squad near fire position
											{	
												if (!isnull _x && _side == side _x) then {																								
													if ((round([position _x,_targetPos] call KRON_distancePosSqr)) < (KRON_UPS_safedist * 0.1)) exitwith {_target = objnull;};
												};										
											} foreach KRON_NPCs;																		
										};
									};
