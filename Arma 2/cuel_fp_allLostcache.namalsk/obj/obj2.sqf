...
private ["_pos","_trpos","_cleanpos","_randFuel"];
...
"Respawn" setMarkerPos (getmarkerpos "respawnmark2");
...
[] spawn {waitUntil {sleep 10; (merlin distance (getPos escapeMark)) < 1000;};[nil,nil,"per",rTitleCut,"you have escaped!", "BLACK" , 6] call RE; ["task8", "succeeded"] call SHK_Taskmaster_upd; sleep 5; [nil,nil,"per",rENDMISSION,"END1"] call RE;};
...
"Respawn" setMarkerPos (getPos sebjanMineLoc);
sleep 3;

[]spawn {
private ["_cpos"];
{
	_cpos = (position evacLoc);
	if((_x distance _cpos < 800) && (side _x == resistance)) then {
		deleteVehicle _x;
	};
	}forEach allUnits;
	{
...

{
	...
	if((_x distance _cleanpos < 500) && (side _x == resistance)) then {
		deleteVehicle _x;
	};
}forEach allUnits;
{
