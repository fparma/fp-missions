...
private ["_genpos","_markpos","_marker","_z"];
...
"Bo_GBU12_LGB" createVehicle (getmarkerPos "fuelMark1");
...
"Respawn" setMarkerPos (getPos armyLoc);
...
"Land_Antenna" createVehicle [(_genpos select 0)+3,( _genpos select 1),(_genpos select 2)];

_markpos = [_genpos,(150-(round(random(150))))] call SHK_pos;

sleep 10;
[nil, nil, rHINT, "You've received a message from the navy. You need to activate their generator to power the antenna and then find the communications tower and send the a broadcast message. Grid coordinates received. Boats can be found at grid 022 043"] call RE;
sleep 2;
...
["task3","Activate antenna","You were contacted by the Military at the Army Base. Since there's no military to be found at the base, it might be a good idea to solve whatever problems the military are having right now.<br><br/></br/>Locate the generator and antenna, and activate it."] call SHK_Taskmaster_add;	
...
"Respawn" setMarkerPos (getmarkerPos "respawnmark1");
	while {!towerVar && alive tower} do {sleep 6};
	"Respawn" setMarkerPos (getmarkerpos "radiomark1");
	if (!alive tower) then {
...
"Respawn" setMarkerPos (getPos evacLoc);
